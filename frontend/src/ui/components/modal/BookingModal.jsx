...
export default function BookingModal({ open, onClose, car, onBookingSuccess }) {
  ...
  const fetchAvailableDates = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/cars/${car.car_id}/available-dates`
      );
      ...
      if (response.ok) {
        ...
        if (data.next_available_date) {
          const nextDate = new Date(data.next_available_date)
            .toISOString()
            .split('T')[0];
  ...
  const updateDriversAvailability = async () => {
    ...
    if (formData.selectedDriver) {
      ...
      const selectedDriver = updatedDrivers.find(
        (d) => (d.drivers_id || d.driver_id) === selectedDriverId
      );

      if (selectedDriver && !selectedDriver.isAvailable) {
        setFormData((prev) => ({ ...prev, selectedDriver: '' }));
        setError(
          '⚠️ The previously selected driver is not available for the chosen dates. Please select another driver.'
        );
      }
    }
  };
  ...
  const handleInputChange = (field, value) => {
    ...
    if ((field === 'startDate' || field === 'endDate') && value) {
      ...
      if (field === 'startDate') {
        ...
        // endDate
        bookingStartDate = formData.startDate
      ...
      if (bookingStartDate && bookingEndDate) {
        ...
        if (conflictingPeriod) {
          ...
          setError(
            `❌ Your selected dates conflict with an unavailable period (${startDateStr} - ${endDateStr}). ${conflictingPeriod.reason}. Please choose dates that don't overlap with blocked periods.`
          );
          ...
          if (
            ...
            error.includes('unavailable period')
          ) {
            setError('');
            setMissingFields([]);
          }
    ...
    if (field === 'dropoffTime' && value) {
      ...
      const isValidDropoffTime =
        (dropoffTimeInMinutes >= minTime && dropoffTimeInMinutes < maxTime) ||
        dropoffTimeInMinutes === 0;

      if (!isValidDropoffTime) {
        setError(
          '❌ Drop-off time must be between 7:00 AM and 12:00 AM (Midnight). Please select a valid time.'
        );
  ...
  const validateForm = () => {
    ...
    if (startDate < minDate) {
      ...
      if (fieldRefs.startDate.current) {
        fieldRefs.startDate.current.scrollIntoView({
          behavior: 'smooth',
          block: 'center',
        });
      }
    ...
    if (endDate < startDate) {
      ...
      if (fieldRefs.endDate.current) {
        fieldRefs.endDate.current.scrollIntoView({
          behavior: 'smooth',
          block: 'center',
        });
      }
    ...
    for (const period of unavailablePeriods) {
      ...
      const hasOverlap =
        bookingStartDate <= periodEnd && bookingEndDate >= periodStart;

      if (hasOverlap) {
        const startDateStr = periodStart.toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric',
          year: 'numeric',
        });
        const endDateStr = periodEnd.toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric',
          year: 'numeric',
        });
        ...
        setError(
          `❌ Your booking dates conflict with an unavailable period: ${startDateStr} - ${endDateStr}. ${period.reason}. Please choose different dates that don't overlap with the blocked periods shown above.`
        );
        setMissingFields(['startDate', 'endDate']);
        setHasDateConflict(true);
    ...
    if (formData.dropoffTime) {
      ...
      if (!isValidDropoffTime) {
        ...
        if (fieldRefs.dropoffTime.current) {
          fieldRefs.dropoffTime.current.scrollIntoView({
            behavior: 'smooth',
            block: 'center',
          });
        }
    ...
    if (formData.pickupTime && formData.dropoffTime) {
      ...
      if (startDateOnly.getTime() === endDateOnly.getTime()) {
        ...
        if (dropoffTimeInMinutes <= pickupTimeInMinutes) {
          setError(
            'Drop-off time must be after pickup time for same-day bookings'
          );
          setMissingFields(['dropoffTime']);
          if (fieldRefs.dropoffTime.current) {
            fieldRefs.dropoffTime.current.scrollIntoView({
    ...
    if (bookingDate.getTime() === today.getTime() && formData.pickupTime) {
      ...
      if (pickupDateTime < threeHoursFromNow) {
        ...
        if (fieldRefs.pickupTime.current) {
          fieldRefs.pickupTime.current.scrollIntoView({
            behavior: 'smooth',
            block: 'center',
          });
        }
  ...
  const getSelectedDriver = () => {
    return drivers.find(
      (driver) => driver.driver_id === formData.selectedDriver
    );
  };
            ...
            >
              ...
              >
                ...
                <Box sx={{ flex: { xs: 'none', md: '0 0 200px' } }}>
                  <Box
                    sx={{
                      ...
                      backgroundImage: car.car_img_url
                        ? `url(${car.car_img_url})`
                        : 'none',
                ...
                <Box sx={{ flex: 1 }}>
                  ...
                  <Typography
                    variant="h6"
                    sx={{ color: '#c10007', fontWeight: 'bold', mb: 2 }}
                  >
              ...
              >
                ...
                <Box>
                  <Typography variant="body1" color="success.main">
                    You can book this car for dates that don't conflict with
                    existing bookings.
                  </Typography>
                </Box>
              </Box>
            ...
            {unavailablePeriods && unavailablePeriods.length > 0 && (
              ...
              >
                ...
                <Typography variant="body2" sx={{ mb: 2 }}>
                  This car has existing bookings or scheduled maintenance. You
                  can still book this car, but{' '}
                  <strong>not during these periods</strong>:
                </Typography>
                ...
                <Alert severity="info" sx={{ mt: 2 }}>
                  <Typography variant="body2">
                    <strong>💡 Tip:</strong> Choose dates that don't overlap
                    with the periods above. The system will notify you if
                    there's a conflict.
                  </Typography>
                </Alert>
              </Box>
                  ...
                  <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      ...
                      <Box sx={{ flex: 1 }}>
                        ...
                        >
                          We'll deliver the car to your location and pick it up
                          when you're done
                        </Typography>
                      </Box>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      ...
                      <Box sx={{ flex: 1 }}>
                        ...
                        <Typography
                          ...
                          sx={{ fontSize: { xs: '0.8rem', sm: '0.875rem' } }}
                        >
                          Pick up the car from our office and return it when
                          you're done
                        </Typography>
                      </Box>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              {activeTab === 0 ? (
                <>
                  ...
                  <Box
                    ref={fieldRefs.deliveryLocation}
                ...
                <>
                  ...
                  <Box
                    sx={{ p: 3, backgroundColor: '#f0f8ff', borderRadius: 2 }}
                  >
                    <Typography
                      variant="h6"
                      sx={{ mb: 1, fontWeight: 'bold', color: '#c10007' }}
                    >
                      📍 Pickup Location
                    </Typography>
              ...
              {formData.purpose === 'Others' && (
                <Box sx={{ position: 'relative' }}>
                  <TextField
                    ...
                    onChange={(e) =>
                      handleInputChange('customPurpose', e.target.value)
                    }
                    multiline
                    rows={3}
                    required
                    ...
                    <Switch
                      ...
                      onChange={(e) => {
                        if (!hasDriverLicense && e.target.checked) {
                          setError(
                            'You cannot select self-drive service because you do not have a driver license on file. Please add your driver license in your account settings or choose a driver.'
                          );
                          return;
                        }
                      ...
                      return (
                        <Card
                          ...
                          sx={{
                            ...
                            border:
                              formData.selectedDriver === String(driverId)
                                ? '2px solid #c10007'
                                : missingFields.includes('selectedDriver')
                                  ? '2px solid #d32f2f'
                                  : isUnavailable
              ...
              {!isSelfService &&
                ...
                drivers.length > 0 && (
                  <Alert severity="warning">
                    ⚠️ No drivers are available for the selected dates. All
                    drivers have conflicting bookings. Please choose different
                    dates or use self-drive service.
                  </Alert>
                )}

              {!isSelfService && drivers.length === 0 && (
                <Alert severity="info">
                  No drivers are currently registered in the system. Please try
                  self-drive service or contact support.
                </Alert>
              )}
            </Box>
          </Box>
        ...
        {activeStep === 2 && (
          <Box sx={{ px: { xs: 2, sm: 3 } }}>
            ...
            >
                ...
                <Card sx={{ flex: 1 }}>
                  <CardContent>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {isSelfService
                        ? '🚗 Self-Drive'
                        : `👨‍💼 With Driver: ${getSelectedDriver()?.first_name} ${getSelectedDriver()?.last_name}`}
                    </Typography>
                  </CardContent>
                </Card>
                  ...
                  {activeTab === 0 ? (
                    ...
                    >
                      <Box>
                        <Typography
                          variant="body2"
                          sx={{ fontWeight: 'bold', color: '#c10007', mb: 0.5 }}
                        >
                          Delivery Address:
                        </Typography>
                        <Typography variant="body1">
              ...
              {!isSelfService && getSelectedDriver() && (
                ...
                >
                  <CardContent>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <Avatar
                        sx={{ bgcolor: '#c10007', width: 56, height: 56 }}
                      >
                        {getSelectedDriver().first_name[0]}
                        {getSelectedDriver().last_name[0]}
                      </Avatar>
                      <Box sx={{ flex: 1 }}>
                        <Typography variant="h6" sx={{ fontWeight: 'bold' }}>
                          {getSelectedDriver().first_name}{' '}
                          {getSelectedDriver().last_name}
                        </Typography>
                        <Typography
                          variant="body2"
                          color="text.secondary"
              ...
              {/* Cost Summary */}
              <Card
                sx={{ backgroundColor: '#f0f8ff', border: '3px solid #c10007' }}
              >
                <CardContent>
                  <Typography
                    variant="h6"
                  ...
                  <Box sx={{ mb: 3 }}>
                    ...
                    >
                      <Typography variant="body2">
                        ...
                        days × ₱{car.rent_price?.toLocaleString()}/day
                      </Typography>
              ...
              {formData.startDate && paymentDeadlineHours && (
                <Card
                  sx={{
                    backgroundColor:
                      formatPaymentDeadline().urgencyLevel === 'error'
                        ? '#ffebee'
                        : formatPaymentDeadline().urgencyLevel === 'warning'
                          ? '#fff8e1'
                          : '#e3f2fd',
                    border: `2px solid ${
                      formatPaymentDeadline().urgencyLevel === 'error'
                      ...
                      <Typography
                        ...
                        sx={{
                          ...
                          color:
                            formatPaymentDeadline().urgencyLevel === 'error'
                              ? '#f44336'
                              : formatPaymentDeadline().urgencyLevel ===
                                  'warning'
                                ? '#ff9800'
                                : '#2196f3',
                        }}
                ...
                new Date(formData.startDate).toDateString() ===
                  ...
                  >
                    <CardContent>
                      ...
                      <Typography variant="body1" sx={{ mb: 2 }}>
                        <strong>Service Availability:</strong> For same-day
                        bookings, the vehicle service will be available 3 hours
                        after payment confirmation.
                      </Typography>

                      <Typography variant="body2" color="text.secondary">
                        This allows time for vehicle preparation, cleaning, and
